...
const getTierName = (tier: number): string => {
  ...
  return names[tier] ?? TIER_NAMES[tier] ?? `Tier ${tier}`;
};
...
export const TierProgressBar: React.FC<TierProgressBarProps> = React.memo(({ tier, score }) => {
  ...
  const useReducedEffects = isNativeApp || deviceTier === 'low' || deviceTier === 'low-mid';
  ...
  useEffect(() => {
    if (tier > prevTierRef.current) {
      setTierFlash(!useReducedEffects);
      const t = setTimeout(() => setTierFlash(false), useReducedEffects ? 120 : 900);
      prevTierRef.current = tier;
      return () => clearTimeout(t);
    }
    prevTierRef.current = tier;
  }, [tier, useReducedEffects]);
  ...
  return (
    <div
      style={{
        ...
        boxShadow: tierFlash && !useReducedEffects
      ...
      >
        ...
        <div style={{ position: 'absolute', inset: 0, borderRadius: 4, overflow: 'hidden' }}>
          ...
          <motion.div
            ...
            transition={useReducedEffects ? { duration: 0.12, ease: 'linear' } : { type: 'spring', stiffness: 45, damping: 14 }}
            style={{
              position: 'absolute',
              left: 0, top: 0,
              height: '100%',
