...
export async function getDailySeedFromServer(): Promise<number> {
  try {
    ...
    // Doküman yoksa (Cloud Function henüz oluşturmadıysa)
    // client-side deterministik seed döndür — yazma yapma
    const dateNum = parseInt(today.replace(/-/g, '')); // e.g., 20260319
    return dateNum;
  } catch (error) {
    console.warn('getDailySeedFromServer: Firebase unavailable, using client seed');
